...
export default function WeeklyFinancialForecast() {
  ...
  let activeTab: 'assumptions' | 'forecast' | 'actions' = 'forecast';
  ...
  useEffect(() => {
    if (location.pathname === '/weekly-forecast') {
      navigate('/weekly-forecast/forecast', { replace: true });
    }
        ...
        <div className='p-8'>
          ...
          {activeTab === 'forecast' && (
            <FinancialPerformanceReview
              opWaterfallStages={mockOPWaterfallStages}
...
interface FinancialPerformanceReviewProps {
  opWaterfallStages: OPWaterfallStage[];
  mockBusinessEvents: Array<{
    id: string;
    name: string;
    type: string;
    impact: number;
...
function FinancialPerformanceReview({
    ...
    visibleScenarios.forEach((scenario) => {
      if (!scenario.simulatedWaterfall) {
        ...
        const totalOPImpact =
          ...
          opWaterfallStages[opWaterfallStages.length - 1].value;
  ...
  }, [scenarios, visibleScenarioIds, opWaterfallStages, valueDriverHierarchy]);
  ...
  const chartData = useMemo(() => {
    const baselineData = opWaterfallStages.map((stage, index) => {
      const prevValue = index > 0 ? opWaterfallStages[index - 1].value : 0;
    ...
    return baselineData;
  }, [opWaterfallStages, scenarios, visibleScenarioIds]);
    ...
    <div className='space-y-8'>
          ...
          <ResponsiveContainer
            ...
            <ComposedChart data={chartData}>
              ...
              <Bar
                ...
                stackId='a'
                name='Baseline'>
                {opWaterfallStages.map((stage, index) => {
                  const isBaseline = stage.type === 'baseline';
      ...
      {isScenarioModalOpen && (
        <ScenarioCreationModal
          ...
          valueDriverHierarchy={valueDriverHierarchy}
      ...
      <div className='bg-white rounded-lg border border-gray-200 p-6'>
        <div className='flex items-center justify-between mb-6'>
          <h2 className='text-xl font-semibold text-gray-900'>Value drivers</h2>
        </div>
