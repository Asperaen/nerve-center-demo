...
interface ActionTrackerSidebarProps {
  isOpen: boolean;
  onToggle: () => void;
  hasPendingActions?: boolean;
}

export default function ActionTrackerSidebar({
  isOpen,
  onToggle,
  hasPendingActions = false,
}: ActionTrackerSidebarProps) {
      ...
      <div
        ...
        <button
          ...
          className={`absolute -left-10 top-1/2 -translate-y-1/2 bg-primary-600 text-white p-2 rounded-l-lg hover:bg-primary-700 transition-all ${
            ...
            !isOpen && (hasPendingActions || pendingCount > 0)
          ...
          <span className='relative inline-block'>
            ...
            {/* Notification Badge when there are pending actions */}
          ...
          <div className='h-full flex flex-col'>
            ...
            <div className='p-4 border-b border-gray-200'>
              ...
              <div className='mt-2 text-xs text-gray-600'>
                Total: {actions.length} • Pending: {pendingCount}
              </div>
            </div>
                  ...
                  {statusColumns.map((status) => {
                    ...
                    return (
                      <div
                        ...
                        {/* Column Header */}
                        <div
                          className={`mb-3 px-3 py-2 rounded-lg border-2 ${getStatusColor(
                            status
                          )}`}>
      ...
      <div className='p-3'>
        <div className='flex items-start justify-between mb-2'>
          <button
            ...
            <h4 className='text-xs font-semibold text-gray-900 hover:text-primary-600'>
              {action.title}
            </h4>
          </button>
          <select
            ...
            className={`ml-2 px-1.5 py-0.5 text-[10px] font-medium rounded border-0 focus:ring-2 focus:ring-primary-500 ${getStatusColor(
              action.status
            )} cursor-pointer`}>
        ...
        <div className='flex items-center justify-between text-[10px] text-gray-500 mb-2'>
          <div className='flex items-center space-x-2'>
            <div className='flex items-center'>
              <UserCircleIcon className='w-3 h-3 mr-0.5' />
              <span className='truncate max-w-[100px]'>{action.owner}</span>
            </div>
            <div className='flex items-center'>
              <CalendarDaysIcon className='w-3 h-3 mr-0.5' />
          ...
          <div className='flex items-center space-x-2'>
            {action.comments.length > 0 && (
              <span className='inline-flex items-center text-[10px] text-gray-600'>
                <ChatBubbleLeftRightIcon className='w-3 h-3 mr-0.5' />
              ...
              <div className='space-y-1.5'>
                {action.comments.map((comment) => (
                  <div
                    ...
                    <p className='text-[10px] text-gray-700'>{comment.text}</p>
          ...
          <div>
            ...
            <div className='flex space-x-1.5'>
              <textarea
                ...
                className='flex-1 px-2 py-1 text-[10px] border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent resize-none'
                rows={2}
              />
              <button
                onClick={(e) => {
                ...
                className='px-2 py-1 text-[10px] bg-primary-600 text-white rounded-lg hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed'>
                Submit
              </button>
            </div>
          </div>
